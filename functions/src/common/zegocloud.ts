...
// FIX: Import Buffer to resolve TypeScript 'Cannot find name' errors.
...
/**
 * Generates a ZegoCloud Kit Token for an authenticated user to join a room.
...
export const generateZegoToken = functions.region("asia-south1").https.onCall(async (data, context) => {
  ...
  try {
    ...
    functions.logger.info(`Successfully generated Zego token for user ${uid} in room ${roomId}`);
    return { token };
  } catch (error) {
